 ...
 * <br>
 * Multithreaded support. Initializing the gyro kicks off a periodic read function which
 * asynchronously reads from they gyro in the background. <br>
 * The periodic update function is designed to be called by this background thread. Each time it is
 * called, it gets the latest value from the gyro, performs any filtering required, and integrates
 * the gyro value to calculate an angle. Class-private variables are updated by this function. <br>
 * USAGE:
 * <ol>
 ...
 * <li>When needed, call the read methods to get the most recent angle or rotational rate reading
 * from the sensor.</li>
 * </ol>
 *
 */

public class L3G4200DGyro {
    ...
    /**
     * Nominal Z value. This is populated at startup with the average value of all the reads we take
     * from the gyro.
     */
    ...
    // Since we need three samples, this keeps track of state in our really simple state machine
    // that
    ...
    // Notice how they're symmetric (first and last are the same, etc.). There's more math behind
    // that!
    //
    // You have to pick these nubmbers very carefully to eliminate the high frequencies you don't
    // want
    // but not damage the low frequencies you do want.
    //
    // And by "pick carefully" what I mean is go to some website like
    ...
    private static final double[] FILTER_COEFS = {0.0023092487065988227, 0.014096933201830508, 0.01882863576981003,
            0.02638335286474602, 0.029521978086505703, 0.027134360076108553, 0.018569615380437842, 0.005598205417752613,
            -0.008101003513074865, -0.01797433887398908, -0.020258649100198693, -0.013651315575190996,
            -0.00020833142072014616, 0.014965417634094582, 0.025336496823333894, 0.025436882849386642,
            0.01331851701527296, -0.007998829315667583, -0.03080398758687758, -0.044851058492634356,
            -0.040775023377656615, -0.013618934982137149, 0.03477631720424106, 0.09551396923065877, 0.1548091828592016,
            0.19794181683885595, 0.2137118638623725, 0.19794181683885595, 0.1548091828592016, 0.09551396923065877,
            0.034776317204241076, -0.013618934982137149, -0.040775023377656615, -0.044851058492634356,
            -0.03080398758687758, -0.007998829315667583, 0.01331851701527296, 0.025436882849386642,
            0.025336496823333894, 0.014965417634094582, -0.0002083314207201463, -0.013651315575190996,
            -0.020258649100198693, -0.017974338873989075, -0.008101003513074865, 0.005598205417752613,
            0.018569615380437842, 0.027134360076108553, 0.029521978086505696, 0.026383352864746017, 0.01882863576981003,
            0.014096933201830508, 0.0023092487065988227,};

    // To calculate the weighted average, we will use a "circular buffer" to hold the previous
    // values of the gyroscope.
    // A circular buffer is a fancy schmancy term for an array where, when you try to index past one
    // end of the array, it
    ...
    // Now, each time we read the gyro, we will want to place the new gyro value into the circular
    // buffer
    ...
    //
    // So, to do this, we introduce the "Circular buffer". It's like a queue. However think of it
    // like this:
    // In a standard queue (waiting line), when we want to add someone new to it, we put them in the
    // back of the line, and
    // make everyone else shift one spot (discarding the oldest guy). But what if that waiting line
    // was actually shaped like
    // a circle, and we just stuck a flag next to the person who was in the front of the line? That
    // way, every time we have
    // a new person to put into the line, we just move the "front of the line" flag toward the
    // person who has been waiting
    // the longest, kick the out of the line, and put the new guy by that flag. Now, the guy who's
    // right next to the flag
    // is the newest to the line, the guy behind him is the second newest, and the guy in front of
    // him is actually the oldest.
    //
    // Hopefully that made some sense. Anyway, that's what we're doing here. This next variable just
    // represents that little
    ...
    // is, it's pretty easy to see why outliers on your input will never go to the output of a
    /////////////////////////////////////////////////////////////////////////////// median filter.
    //
    ...
    // I'm assuming that MED_FILT_LEN is fairly small, so I didn't bother with a circular buffer on
    // this one.
    ...
    /**
     * Constructor initalizes the data associated with the gyro, starts talking to it over I2C It
    ...
    L3G4200DGyro() {
        ...
        filter_buf_start_pointer = 0; // initalize the poitner to an aribtrary location. I like
                                      // zero. It's circular, so it doesn't actually matter.
        ...
        // sleep briefly to ensure settings take effect
        // probably not actually required, but doesn't hurt either
        try {
            Thread.sleep(10); // in ms
        } catch (InterruptedException e) { // java requires us to catch exceptions, don't know why
                                           // exactly...
    ...
    /**
     * Get the most recent angle calculation result from the gyro. Angle can vary between -Infinity
     * and Infinity, you must wrap this to 0-360 if desired.
     * 
    ...
    private short read_gyro_z_reg() {
        ...
        byte[] buffer_config_test_vals = {0}; // buffer to test the config values to make sure we
                                              // didn't hit brownout
        ...
        if (buffer_config_test_vals[0] == (byte) CTRL_REG1_CONTENTS) {
            ...
            System.out.println("ERROR BAD READ FROM GYRO. Got " + buffer_config_test_vals[0]
                    + " for CtrlReg1, expected " + CTRL_REG1_CONTENTS);
        ...
        // The desired 16-bit reading is split into two eight-bit bytes in memory and over I2C
        // com's. This line just recombines those
        // two bytes into a 16bit number, and applies the zero-motion-offset.
        // Typecasting magic here, don't touch!
        short ret_val = (short) (((buffer_low_and_high_bytes[1] << 8) | (buffer_low_and_high_bytes[0] & 0xFF))
                - (short) zero_motion_offset);
    ...
    private void periodic_update() {
        ...
        gyro_z_val_deg_per_sec[0] = gyro_median_filter((double) read_gyro_z_reg() * degPerSecPerLSB); // Read
                                                                                                      // new
                                                                                                      // value,
                                                                                                      // scale,
                                                                                                      // and
        ...
        if (integration_method == 1) {
            if (cur_interrupt_state == 0) { // initalize variables on first call of asynchronous
                                            // function
            ...
            } else if (cur_interrupt_state == 2) { // We've got three samples, go ahead and
                                                   // calculate!
                long cur_period_ns = (cur_period_start_time - system_time_at_last_call); // Calculate
                                                                                         // the
                                                                                         // amount
                                                                                         // of time
                                                                                         // since
                ...
                angle = angle + (double) cur_period_ns / (double) 1000000000 * 1 / 6
                        * (gyro_z_val_deg_per_sec[2] + 4 * gyro_z_val_deg_per_sec[1] + gyro_z_val_deg_per_sec[0]); // simpson's
                                                                                                                   // method
        ...
        // But if we're using a linear method...
            ...
            long cur_period_ns = (cur_period_start_time - system_time_at_last_call); // Calculate
                                                                                     ...
                                                                                     // calculated
                                                                                     // an integral
            angle = angle + (double) cur_period_ns / (double) 1000000000 * 1 / 2
                    * (gyro_z_val_deg_per_sec[0] + gyro_z_val_deg_per_sec[1]); // calculate integral
                                                                               // using linear
                                                                               // method
            system_time_at_last_call = cur_period_start_time; // record the current sample time for
                                                              // usage next time we calcualte an
                                                              // integral
        }
    }
    ...
    private double gyro_LP_filter(double input) {
        ...
        for (int i = 0; i < FILTER_LENGTH; i++) {
            ...
            // Add up all of the current buffer values, multiplied by the approprate weight
            // (coefficent)
    ...
    private class GyroTask extends TimerTask {
        private L3G4200DGyro m_gyro;


        public GyroTask(L3G4200DGyro gyro) {
            if (gyro == null) {
